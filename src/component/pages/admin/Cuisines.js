...
const Cuisines = ({ history }) => {
  ...
  const activedeactive = async (id, status) => {
    ...
    const result = await axios.put(`http://${process.env.REACT_APP_IP}/api/cuisines/${id}/${newstatus}`, '', '');
            ...
            <ThemeProvider theme={defaultMaterialTheme}>
              <MaterialTable
                ...
                columns={[
                  ...
                  { title: 'CuisinesImage', field: 'imageUrl', render: rowData => <a href={`http://${process.env.REACT_APP_IP}${rowData.cuisinesImage}`} target="_blank"><img src={`http://${process.env.REACT_APP_IP}${rowData.cuisinesImage}`} alt="CuisinesImage" style={{ width: 40, borderRadius: '50%' }} /></a> },
                  { title: 'CuisinesBanner', field: 'imageUrl', render: rowData => <a href={`http://${process.env.REACT_APP_IP}${rowData.cuisinesBanner}`} target="_blank"><img src={`http://${process.env.REACT_APP_IP}${rowData.cuisinesBanner}`} alt="Cuisines Banner" style={{ width: 60, height: 30 }} /></a> },
                  { title: 'CuisinesName', field: 'cuisinesName' },
                  { title: 'Description', render: rowData => Description(rowData.cuisinesDescription)},
                  { title: 'Added On', render: rowData => rowData.createdAt.split('T')[0]  },
                  {
                    title: 'Cuisines Status', render: rowData => <div style={{ display: 'flex' }}>

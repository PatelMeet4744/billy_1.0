...
export const getreferralAmount = () => async (dispatch) => {
    try {
      ...
      const { data } = await axios.get(`http://${process.env.REACT_APP_IP}/api/referralAmount?page=1&pageSize=10`, '', '');
...
export const updateReferralAmount = (referralAmountId, referralAmount) => async (dispatch) => {
    try {
      ...
      const token = ("bearer " + await sessionStorage.getItem("x-auth-token"));
      axios.defaults.headers.common["Authorization"] = token;
      const {data} = await axios.put(`http://${process.env.REACT_APP_IP}/api/referralAmount/${referralAmountId}`, referralAmount, config);

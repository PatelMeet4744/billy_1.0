...
export const getquestion = () => async (dispatch) => {
    try {
        ...
        const { data } = await axios.get(`http://${process.env.REACT_APP_IP}/api/question`, '', '');
...
export const createQuestion = (questionData) => async (dispatch) => {
  try {
    ...
    const token = ("bearer " + await sessionStorage.getItem("x-auth-token"));
    axios.defaults.headers.common["Authorization"] = token;
    const { data } = await axios.post(`http://${process.env.REACT_APP_IP}/api/question`, questionData, config);
...
export const deletequestion = (id) => async (dispatch) => {
    try {
      ...
      const token = ("bearer " + await sessionStorage.getItem("x-auth-token"));
      axios.defaults.headers.common["Authorization"] = token;
      const { data } = await axios.delete(`http://${process.env.REACT_APP_IP}/api/question/${id}`, '', config);
      if(data.message === "Success")
      {
        dispatch({
...
export const updatequestion = (id, questionData) => async (dispatch) => {
  try {
    ...
    const token = ("bearer " + await sessionStorage.getItem("x-auth-token"));
    axios.defaults.headers.common["Authorization"] = token;
    const {data} = await axios.put(`http://${process.env.REACT_APP_IP}/api/question/${id}`, questionData, config);
...
export const getsinglequestion = (id) => async (dispatch) => {
  try {
      ...
      const token = ("bearer " + await sessionStorage.getItem("x-auth-token"));
      axios.defaults.headers.common["Authorization"] = token;
      const { data } = await axios.get(`http://${process.env.REACT_APP_IP}/api/question/${id}`, '', config);

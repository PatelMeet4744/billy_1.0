  ...
  const { error: deleteError, isDeleted } = useSelector(
    ...
    const activedeactivevariantStatus = async (id, status) => {
      ...
      if (result.data.message === "Success") {
        ...
        dispatch(getvariant(restaurantId));
        // getScopes();
      } else {
        swal({
          title: "Variant",
              ...
              <ThemeProvider theme={defaultMaterialTheme}>
                <MaterialTable
                  ...
                  },
                  {
                      ...
                      <div style={{ display: 'flex' }}>
                          <Link to={"/Partner/Variant/ManageVariant/" + rowData.variantId}><a><label class="badge badge-success hand_cursor">Edit</label></a></Link>&nbsp;
                        <a onClick={()=>deletevariantHandler(rowData.variantId)}><label style={{cursor:'pointer'}} class="badge badge-danger delete_red hand_cursor">Delete</label></a>
                      </div>
                    },
                  ]}
                  data={

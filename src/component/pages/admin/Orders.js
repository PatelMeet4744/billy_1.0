...
const Orders = ({ history }) => {
    ...
    const {
      ...
      useEffect(() => {
        dispatch(getorders());
        // return alert(JSON.stringify(Orders.orderMaster))
        ...
        <div className="card-body">
          ...
          <ThemeProvider theme={defaultMaterialTheme}>
            <MaterialTable
              ...
              title="Orders"
              columns={[
                { title: 'S.No #', render: rowData => <Link to={"orderdetail/" + rowData._id} className='add_link'><button style={{backgroundColor:'white',border:'none',color:'blue'}}>{rowData.tableData.id+1}</button></Link> },
                ...
                { title: 'Total', render: rowData => rowData.orderMaster.orderTotalPrice},
                { title: 'Payment Status', render: rowData => rowData.orderMaster.orderPaymentStatus},
                { title: 'Qty', render: rowData => rowData.orderQty},
                
                { title: 'Added On', render: rowData => rowData.orderMaster.createdAt.split('T')[0] },

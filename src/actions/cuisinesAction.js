...
export const getcuisines = () => async (dispatch) => {
    try{
        ...
        const { data } = await axios.get(`http://${process.env.REACT_APP_IP}/api/cuisines`, '', '');
...
export const deletecuisines = (id,cuisinesData) => async (dispatch) => {
    try {
      ...
      const token = ("bearer " + await sessionStorage.getItem("x-auth-token"));
      axios.defaults.headers.common["Authorization"] = token;
      const { data } = await axios.post(`http://${process.env.REACT_APP_IP}/api/cuisines/${id}`, cuisinesData, config);
...
export const createcuisines = (formData) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.post(
      `http://${process.env.REACT_APP_IP}/api/cuisines`,
      formData, config
  );
